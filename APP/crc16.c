...
static unsigned short crc_accum;
...
void CRC16AddByte (unsigned char newbyte) {
   ...
   unsigned char tblidx = newbyte ^ (unsigned char)crc_accum;
   crc_accum >>= 8;
   crc_accum ^= CRC16Table[tblidx];
}
...
unsigned short CRC16Calc (const unsigned char *pucBuff, unsigned long ulLen) {
   ...
   CRC16Init();
   CRC16AddBytes(pucBuff,ulLen);
   return CRC16GetValue();
}
